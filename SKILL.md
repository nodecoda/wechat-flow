...
搜索顺序（高优先在前）：
  ...
  {skill_dir}/config.yaml         ← skill 全局配置
  {skill_dir}/toolkit/config.yaml
...
python3 {skill_dir}/toolkit/cli.py publish {markdown} --cover {cover} --theme {theme} --title "{title}" --digest "{digest}" --account {target_account}

# 查看已配置账号（--account 可选值）
python3 {skill_dir}/toolkit/cli.py accounts

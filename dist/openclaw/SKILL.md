...
**路径约定**：本文档中 `{baseDir}` 指本 SKILL.md 所在的目录（即 NCoda 的根目录）。
...
- 用户说"更新"/"更新 NCoda"/"升级" → 在 `{baseDir}` 执行 `git pull origin master`，完成后告知版本变化
...
比对本地 `{baseDir}/VERSION` 与远程 `git show origin/master:VERSION`：
- 相同 → 静默通过

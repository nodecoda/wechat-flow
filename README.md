...
它不是一个传统意义上的单体应用，而是一个以 `SKILL.md` 为编排核心、以 `toolkit/` 和 `scripts/` 为执行底座、以 `references/`、`personas/`、`themes/` 为知识层的内容生产系统。项目目标不是单次“生成一篇文章”，而是把公众号内容生产做成一个可持续优化的工作流：

热点抓取 → 选题 → 框架 → 素材采集 → 内容增强 → 写作 → SEO → 生图 → 微信排版 → 推送草稿箱 → 数据回填 → 修改学习
...
| 层级 | 作用 | 关键文件 |
|------|------|---------|
| 编排层 | 定义 Step 1-8 主流程、交互模式、降级策略与收尾协议 | `SKILL.md` |
